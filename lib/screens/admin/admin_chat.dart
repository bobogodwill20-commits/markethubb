...
class _AdminChatScreenState extends State<AdminChatScreen> with SingleTickerProviderStateMixin {
  ...
  Future<void> _startNewConversation(String userId) async {
    final currentUser = Supabase.instance.client.auth.currentUser;
    if (currentUser == null) return;
    
    try {
      final List<String> ids = [currentUser.id, userId];
      ids.sort();
      final conversationId = ids.join('_');
  ...
  Widget build(BuildContext context) {
    ...
    // On mobile, force sidebar hidden when chat is selected
        ...
        children: [
          ...
          if (!isMobile || (isMobile && _selectedConversation == null))
            AnimatedContainer(
              ...
              child: _showUserList 
                  ? _buildUserList(isDark, isMobile) 
                  : _buildConversationList(isDark, isMobile),
            ),
          // Chat Area
          if (_selectedConversation != null)
            Expanded(
  ...
  Widget _buildUserList(bool isDark, bool isMobile) {
  ...
  Widget _buildUserTile(Map<String, dynamic> user, bool isDark) {
    ...
    return InkWell(
      onTap: () => _startNewConversation(user['id']),
      child: Container(
        padding: const EdgeInsets.symmetric(horizontal: 12, vertical: 10),
        decoration: BoxDecoration(
          border: Border(
  ...
  Widget _buildChatArea(bool isDark, bool isMobile) {
    ...
    return Column(
      children: [
        // Messages - WhatsApp style with date separators
        Expanded(
